  ...
  return <Screen includeBottomSafeArea={false} padded={false} scroll={false}>
    ...
    <View style={styles.screenContent}>
      ...
      <View style={styles.grid}>{buildMonth(year, month).map((day, index) => {
        ...
        const isToday = date === toDateKey(today);
        ...
        return <Pressable key={day} onPress={() => setSelectedDate(date)} style={({ pressed }) => [styles.dayCell, pressed && styles.dayPressed]}>
          <View style={[styles.dayCircle, (isSelected && !isToday) && styles.selectedCircle, isToday && styles.todayCircle, holiday && styles.holidayCircle]}><Text style={[styles.dayText, holiday && styles.holidayText, (isSelected || isToday) && styles.white]}>{day}</Text></View>
          <View style={styles.dotContainer}>
            {appRun && <View style={[styles.dot, styles.completedDot]}/>}
...
const styles = StyleSheet.create({
  ...
  calendarCard: { position: 'absolute', top: 200, left: 24, right: 24, height: 373, borderRadius: 25, backgroundColor: colors.white, paddingHorizontal: 17, paddingTop: 16 }, monthRow: { height: 34, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 21 }, monthButton: { width: 32, height: 32, alignItems: 'center', justifyContent: 'center', borderRadius: 10 }, month: { color: '#1C1A1A', fontSize: 17, fontWeight: '800' }, weekRow: { flexDirection: 'row', marginTop: 4 }, weekday: { width: '14.285%', textAlign: 'center', color: '#686868', fontSize: 12, fontWeight: '700' }, grid: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 4 }, dayCell: { width: '14.285%', height: 44, alignItems: 'center', justifyContent: 'center' }, dayPressed: pressFeedback, dayCircle: { width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, selectedCircle: { backgroundColor: colors.primary }, todayCircle: { backgroundColor: colors.primary }, holidayCircle: { borderWidth: 1.5, borderColor: '#D94B4B' }, dayText: { color: '#1C1A1A', fontSize: 13, fontWeight: '700' }, holidayText: { color: '#D94B4B' }, white: { color: colors.white }, dotContainer: { position: 'absolute', bottom: 2, flexDirection: 'row', gap: 3, alignItems: 'center' }, dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#1C1A1A' }, completedDot: { backgroundColor: colors.primary }, manualDot: { backgroundColor: '#FFC2B3' }, planDot: { backgroundColor: 'transparent', borderWidth: 1, borderColor: colors.primary }, holidayDot: { backgroundColor: '#D94B4B' }, legend: { position: 'absolute', left: 0, right: 0, bottom: 12, flexDirection: 'row', justifyContent: 'center', gap: 20 }, legendItem: { flexDirection: 'row', alignItems: 'center', gap: 5 }, legendDot: { width: 7, height: 7, borderRadius: 4, borderWidth: 1 }, legendText: { color: '#686868', fontSize: 11 },
