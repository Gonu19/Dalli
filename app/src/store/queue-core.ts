...
export function createQueue(storage: QueueStorage, now: () => string = () => new Date().toISOString()) {
  ...
  function list(): QueuedRun[] {
    return [...storage.list()].sort((a, b) => a.savedAt.localeCompare(b.savedAt));
  }
  ...
  return {
    ...
    enqueue(record: RunRecord): void {
      storage.write({ record, attempts: 0, lastError: null, savedAt: now() });
    },
    ...
    markFailed(clientRunId: string, error: unknown): void {
      const entry = storage.list().find((item) => item.record.clientRunId === clientRunId);
