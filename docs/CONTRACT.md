...
- 캘린더 날짜 경계는 `Asia/Seoul`을 사용한다. `started_at`은 UTC로 저장하되 KST 날짜로 묶는다.
- 계획과 러닝이 모두 없는 날짜는 응답에서 생략하고, 날짜와 러닝은 오래된 순으로 정렬한다.
...
- 통계 날짜 경계는 `Asia/Seoul`이다.
- 주간은 월요일 00:00부터 다음 월요일 00:00 전까지이며, `this_week_count`는 고유 러닝 날짜 수다.
- 저장된 APP/MANUAL 러닝은 `completed`와 관계없이 포함한다.
- `next_milestone`은 현재 누적 러닝일보다 큰 다음 10일 단위 값이다. 최대 단계는 두지 않는다.
...
| 캘린더·통계 날짜 | `Asia/Seoul` 날짜 경계 |
| 주간 집계 | 월요일 시작, 고유 러닝 날짜 수 |
| 통계 포함 러닝 | 저장된 APP/MANUAL 전체(미완료 포함) |
| 다음 마일스톤 | 누적보다 큰 다음 10일 단위, 최대 없음 |
