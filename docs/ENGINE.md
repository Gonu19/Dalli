...
| `TOO_SLOW` 문구 전환 | **5분(300초)** | `SLOW_JUDGE_START_SEC` |
| 5분 이전 `TOO_SLOW` 상한 | **2회** | `MAX_EARLY_SLOW_INTERVENTION` |
...
| **초기 목표값 기준** | 경험 `0`/`1`/`2` → **140 / 150 / 160** (§2) | `EXPERIENCE_BASE_CADENCE` |
...
| --- | --- | --- |
| `0` | 최근 달린 적이 거의 없음 | **140** |
| `1` | 가끔 달림 | **150** |
| `2` | 꾸준히 달림 | **160** |

> **2026-08-20 하향 (152 / 158 / 164 → 140 / 150 / 160).** 실기기 검증에서 초보에게
> 150대가 빨랐다. 실제 러너의 평균이 133spm이었고 엔진이 다운시프트로 목표를 140까지
> 낮췄다 — **엔진이 이미 140으로 수렴시키고 있었다는 뜻**이라 출발점이 틀린 것이다.
>
> 그대로 두면 초보의 첫 러닝이 이렇게 흘러간다: 목표에 못 미침 → 저속 안내 → 실패 2회 →
> 목표 하향 → 또 하향. **완주를 돕겠다는 앱에서 첫 경험이 계속되는 지적이 된다.**
>
> 낮추는 폭을 경험별로 다르게 준 이유: 러닝 중 목표 상향은 §7에서 금지다. 꾸준히 달리는
> 사람에게까지 크게 낮추면 러닝 내내 과속 판정을 받고 되돌릴 방법이 없다. 그래서
> 초보는 −12, 가끔은 −8, 꾸준히는 −4로 폭을 좁혔다.
>
> **이 값이 흔들리는 건 1회차뿐이다.** baseline은 목표와 무관하게 90~270초 구간 실측
> 중앙값으로 잡히므로(§2), 2회차부터는 그 사람의 실제 리듬이 기준이 된다.
...
러닝 종료 → 업로드 → 서버가 samples에서 산출해 users에 저장 → 리포트가 알려준다
...
**`POST /runs` 처리 중 서버가 계산해 `users.baseline_cadence`에 저장한다.**
사용자에게 묻지 않고, 클라이언트는 `PATCH`하지 않는다.

- **비어 있을 때만 자동 확정한다.** 즉 첫 유효 러닝 1회. 이후 목표 변경은
  세션 간 업시프트(아래)와 러닝 중 하향(§8)으로만 일어난다.
  매 러닝 덮어쓰면 리포트가 제안한 `+2`가 다음 업로드에서 지워진다
- 조건을 못 채운 러닝(6분 미만·유효 샘플 30개 미만)은 그냥 넘어간다. 다음 러닝이 채운다

사용자에게 확인을 받지 않는 이유가 둘이다.
- **판단할 수 있는 질문이 아니다.** 자기 실측값을 거부한다는 것이 무슨 뜻인지
  사용자가 알 수 없다. 화면만 하나 늘어난다
- **확인 버튼은 유실 지점이다.** 리포트를 안 보고 나가거나 앱을 닫으면 그 러닝의
  실측이 통째로 버려지고, 다음 러닝도 온보딩 추천값으로 돈다.
  20분을 뛰고도 기준이 안 잡히는 동작은 만들지 않는다

클라이언트도 같은 규칙으로 계산하지만 **화면 표시 전용**이다(`runStore`의 `measuredBaseline`).
업로드가 큐에 머무는 동안에도 "기준 리듬을 찾았어요 — 157"을 보여주기 위해서고,
저장 권한은 서버에만 있다.
...
| 대상 | 노출 |
| --- | --- |
| baseline 숫자 | **○** — 첫 러닝 리포트("기준 리듬을 찾았어요 — 157"), 리포트 하단, 설정. **알림이지 확인 요청이 아니다** |
| 러닝 중 화면의 baseline | ✕ |
...
## 4. 측정 사이클
```
1초 타이머 : 최근 8초 누적 걸음 증가분 → SPM
20초 윈도우: 중앙값 → 판정용 cadence
 6초 윈도우: 중앙값 → 화면 표시용 cadence
5초 tick   : 판정 + samples push
```

### 화면 표시용 윈도우를 따로 두는 이유 ⚠️

판정용 20초 중앙값을 화면에도 쓰면 **리듬을 올려도 숫자가 한참 안 변한다.** 중앙값은
새 값이 표본의 과반이 되기 전까지 움직이지 않아서, 10초쯤 멈춰 있다 갑자기 튄다.
실기기 검증에서 "빨리 뛰는데 숫자가 안 변한다"는 보고가 나온 원인이 이것이다.

그래서 화면은 `DISPLAY_WINDOW_SEC`(6초) 중앙값을 쓴다. **판정은 20초 중앙값 그대로다** —
개입·다운시프트·리커버리 규칙은 하나도 바뀌지 않는다.

숫자가 색보다 먼저 움직이는 것은 의도한 동작이다. 숫자는 지금 상태를 즉시 보여주고,
색은 그 상태가 이어질 때(§7의 지속 조건) 바뀐다. `samples`에 저장하는 값도 판정용이다.
- `watchStepCount` 콜백에서는 누적값만 갱신 (호출 빈도 불규칙).

### 1초 차분을 쓰지 않는 이유 ⚠️

iOS는 걸음을 **묶어서** 올려보낸다. 3초 조용하다가 한 번에 5걸음이 들어오는 식이다.
1초 차분으로 SPM을 만들면 `0, 0, 300, 0, 0, 240 …`이 되고, 그 값의 20초 중앙값은
**0으로 내려앉는다** — 걷고 있는데 화면이 0을 가리킨다. 실기기에서 실제로 겪은 증상이고,
`step-rate-check`가 그 상황을 재현해 확인한다.

그래서 소스는 **최근 8초 구간의 누적 증가분**을 구간 길이로 나눈다. 배치로 들어와도
구간 합은 같아 값이 안정적이다. 8초인 이유는 짧으면 배치에 다시 흔들리고, 길면 리듬
변화가 늦게 보이기 때문이다 — 뒤의 20초 윈도우가 한 번 더 다듬는다.

누적값의 출처는 둘이다. `watchStepCount` 구독과 `getStepCountAsync` 조회 중 **큰 값**을
쓴다. 구독이 조용한 기기가 있어서, 어느 한쪽이 죽어도 측정이 멈추지 않게 한다.
...
| --- | --- | --- | --- |
| 0 ~ 90초 | ✕ | ✕ | ✕ |
| 90초 ~ 5분 | **○** | **○ (2회까지, 문구 다름)** | `TOO_SLOW`·걷기만 |
| 5분 이후 | ○ (2회 소진 전) | ○ | `TOO_SLOW`·걷기만 |
| 목표 90% 이후 | ✕ | ○ | ○ |
| 리커버리 진입 후 | ✕ | ✕ | ✕ |

### 5분 이전 저속은 재촉이 아니라 리듬으로 답한다 ⚠️

목표를 세워두고 리듬이 처지는데 5분을 통째로 침묵하면, 러닝의 앞 구간을 방치하게 된다.
5분도 못 버티고 무너지는 초심자에게는 **그 구간이 러닝의 전부**이기도 하다.

그렇다고 초반 저속에 "처지고 있어요"라고 말하면 아직 몸 푸는 사람을 재촉하는 것이다.
그래서 **말의 성격을 바꾼다.**

| | 문구 | 메트로놈 | 횟수 |
| --- | --- | --- | --- |
| 90초 ~ 5분 | `목표 리듬을 들려드릴게요.` | ○ | **2회** |
| 5분 이후 | `리듬이 조금 처지고 있어요. 지금 속도를 유지해 볼까요?` | ○ | 제한 없음 |

초반 구간의 주인공은 메트로놈이다. 말은 판단("당신이 느리다")을 담지만 메트로놈은 리듬만 준다.

2회로 묶는 이유는 그 이상이 재촉이기 때문이고, **2회를 채우면 실패 2회가 되어 그대로
하향으로 이어진다**(§8). 목표가 그 사람에게 높다는 뜻이므로 재촉을 늘리는 대신 목표를 내린다.

### 하향에는 시각 게이트가 없다

5분도 못 버티는 사용자에게 필요한 것은 재촉이 아니라 **낮아진 목표**다. 하향까지 5분간
잠가두면 정작 도와야 할 구간에서 아무것도 하지 못한다.

대신 **RUN 구간을 한 번이라도 지났을 것**을 전제로 둔다(`hasRunZone`). 출발선에서 걸어
이동하거나 몸을 푸는 사람은 시작부터 WALK 구간인데, 그 걷기 60초로 목표를 깎으면
한 발도 뛰기 전에 목표가 내려간다.
...
- 개입 피드백은 **음성 ≤3초 1문장 · 메트로놈 5초 · 진동** 세 채널이며 각각 설정으로 끈다.
  셋 다 `cue-player`가 함께 내보낸다. 무엇을 알릴지는 `engine/cues.ts`의 `Cue`가 정한다
  (`metronome`, `haptic`).
- 진동은 리듬을 고치라는 신호(`impact`)와 상태가 바뀌었다는 알림(`warning`)으로 나뉜다.
  `RECOVERY_MODE_ON`만 `warning`이다.
- **셋을 모두 끄면 아무것도 내보내지 않는다.** 끈 채널을 다른 채널로 대신 울려 주지 않는다.
- **러닝 시작 안내**: `RUN_START`에 목표 리듬 중심값을 한 문장으로 말하고 메트로놈을
  `METRONOME_INTRO_SEC`(10초) 들려준다. 고칠 것이 아니라 기준을 몸에 넣는 자리라 개입보다 길다.
- **기준 리듬 주기 안내**: `PACE_REMINDER_SEC`(60초)마다 메트로놈만 5초. 음성·진동은 쓰지 않는다.
  **그 주기에 개입이 있었으면 건너뛴다** — 지적 직후에 또 울리면 쿨다운이 무의미해지고
  개입 신호가 주기 소리에 묻힌다. pause 중에는 활동 시간이 멈추므로 저절로 미뤄진다.
- 쿨다운 60초: 무음 + 판정 보류.
...
### `TOO_SLOW` — 구간에 따라 문구가 갈린다
| 구간 | 문구 | 이후 |
| --- | --- | --- |
| 90초 ~ 5분 | "목표 리듬을 들려드릴게요." (+ 메트로놈) | 쿨다운 60초. **2회까지**, 2회 뒤 하향 |
| 5분 이후 | "리듬이 조금 처지고 있어요. 지금 속도를 유지해 볼까요?" | 쿨다운 60초. 실패 2회면 하향 |
...
**모든 트리거의 공통 전제**: 이번 세션에 RUN 구간(> 120 spm)을 한 번이라도 지났을 것 (§5).
시각 게이트는 없다 — 워밍업 90초만 지나면 하향이 가능하다.
...
| `POST /runs` 업로드 | ✕ |
| DB (`ERD.md`) | ✕ |
| 결과 이미지 (`F2-10`) | **○** — 활동 시간·거리·평균 리듬과 함께 경로를 그린다 |

서버로 나가는 것은 **누적 거리와 평균 페이스 두 숫자뿐**이다. 그래서 지도를 넣어도
`CONTRACT.md`·`ERD.md`는 바뀌지 않는다.

**결과 이미지는 종료 직후에만 경로를 담을 수 있다.** 좌표가 메모리에만 있으므로,
앱을 껐다 켜거나 과거 러닝을 다시 열면 경로 없이 숫자만 남는다. 이미지 생성은
기기에서 끝나고 좌표가 서버로 가지 않는다는 점은 그대로다.
...
- `HABIT`은 유일하게 **`prescription` 자체가 러닝 밖(다음 러닝 시점)** 을 향한다. 한 번의
  러닝을 잘하는 것보다 다시 나가는 것이 목적이기 때문이다.
...
### 루틴 데이터는 전 목적 공통 입력이다

주간 횟수·직전 러닝과의 간격·계획 이행 수는 **`HABIT` 전용이 아니다.** 리포트 입력에
언제나 넣고, 목적은 그것을 *어디에 쓸지*만 정한다.

러닝 하나만 보고는 판단할 수 없는 것이 있기 때문이다. **부담(FI)이 높은데 간격이 하루면
회복 안내가 달라져야 하고, 부담이 낮은데 닷새를 쉬었으면 그건 루틴이 끊긴 신호다.**
`recovery_note`는 모든 목적에 있는 필드인데, 간격을 모르면 "하루 정도 간격을 두세요" 같은
일반론밖에 못 쓴다 — 사흘 쉬고 나온 사람에게도 같은 말을 하게 된다.

| `running_purpose` | 루틴 데이터의 역할 |
| --- | --- |
| `COMPLETE` | 간격이 짧고 FI가 높으면 완주보다 **회복을 먼저** 말한다 |
| `HABIT` | `prescription`의 본체 — 다음 러닝 시점 |
| `WEIGHT` | 1회 시간이 아니라 **주간 누적 시간**으로 말한다 |
| `FITNESS` | 후반 하락(LDR)을 간격과 함께 읽는다. 짧은 간격의 하락은 피로 신호다 |
| `PERFORMANCE` | 간격은 **회복 관점으로만** 쓴다. 더 자주 달리라고 하지 않는다 |

`evidence`에 루틴 수치를 넣는 것은 `HABIT`뿐이다. 나머지 목적에서는 `recovery_note`와
`prescription`의 판단 근거로만 쓰고 화면에 숫자로 나열하지 않는다 — `evidence`는
**그 러닝의 핵심 수치 1~3개**라는 규칙이 우선한다(`CONTRACT.md`).

### 계획 상태는 저장하지 않고 계산한다 ⚠️

캘린더 계획(`plans`)은 루틴의 기록이자 `HABIT` 피드백의 재료다. 여기서 필요한
"안 지킨 계획"을 **저장 컬럼으로 두지 않고 조회 시 계산한다.** `onboarded`와 같은 방식이다.

| 조건 | `status` |
| --- | --- |
| 연결된 러닝 있음 | `DONE` |
| `planned_date` < 오늘(KST) & 연결 러닝 없음 | **`SKIPPED`** |
| 그 외 | `PLANNED` |

사용자가 직접 `PATCH`한 값은 계산보다 우선한다. 적용 대상은 `GET /plans`·`GET /calendar`.

저장하지 않는 이유가 셋이다.
- **자정마다 뒤집는 배치가 필요 없다.** 안 도는 순간 캘린더가 조용히 거짓말을 한다
- **시간대 경계가 사라진다.** `planned_date`는 KST 날짜인데 서버는 UTC로 돈다.
  조회 시 한 번 비교하면 끝이고, 잘못 박힌 `SKIPPED`가 남지 않는다
- **소급 기록이 반영된다.** 어제 앱 없이 뛴 것을 오늘 수기로 넣으면 어제 계획이 즉시
  `DONE`으로 돌아온다. 저장 방식이면 실패 표시가 박제된다 — 페르소나의 Pain Point가
  *"달성하지 못하면 러닝 자체를 실패로 느낀다"*이므로 되돌아오지 않는 실패 표시는 쓰지 않는다

### 러닝과 계획 연결 — 전후 6시간

계획은 날짜만 갖는다(시각 없음). 러닝 시작 시각이 **계획일 자정 기준 전후 6시간** 안이면
같은 러닝으로 본다. 자정 직전 러닝이 다음 날 계획으로 넘어가거나 새벽 러닝이 전날 계획을
못 채우는 일을 막는다.

- 오늘 계획이 있으면 언제나 오늘이 우선 (날짜당 계획은 하나다)
- 오늘 계획이 없을 때만: `06시 이전 → 어제 계획`, `18시 이후 → 내일 계획`
- 이미 `DONE`·`SKIPPED`인 계획은 연결하지 않는다

클라이언트가 러닝 **시작 시점**에 판단해 `plan_id`를 실어 보낸다. 업로드가 도착하면
서버가 같은 트랜잭션에서 계획을 `DONE`으로 바꾼다(`CONTRACT.md`).
구현은 `store/plan-link.ts`, 상수는 `PLAN_LINK_WINDOW_HOURS`.
