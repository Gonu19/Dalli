...
- 온보딩 수동 조절: **추천값 기준 ±10 spm**, 버튼은 `−5 / −1 / +1 / +5`.
...
**152**가 되고, 사용자는 케이던스 화면에서 조절할 수 있다.

### 조절 화면의 상·하한 ⚠️

버튼이 `±5`까지 있으므로 조절 폭은 **추천값 기준 ±10**(±5를 두 번)이다.
그 위에 **§1의 절대 클램프 130~185가 언제나 우선한다.**

하한을 130 미만으로 열면 안 되는 이유가 분명하다 — 걷기 구간이 `50~120`이므로(§4)
중심값을 120 근처로 두면 **걷는 것이 "목표 달성"이 되어 판정이 통째로 무너진다.**
downshift 하한을 130으로 잡은 것(§8)과 같은 이유다. 상한 185도 같은 표를 쓴다.

상수는 `INITIAL_TARGET_ADJUST_RANGE`(10)와 `INITIAL_TARGET_ADJUST_STEPS`,
계산은 `clampAdjustedTarget(value, recommended)`.
...
같은 러닝이라도 **왜 달리는지에 따라 다음에 할 일이 다르다.** 체중 관리로 시작한
사람에게 "후반 페이스 유지"를 처방하면 남의 목표를 대신 세워주는 셈이 된다.

`users.running_purpose`를 리포트 생성 입력에 포함한다. 사용자와 러닝은 이미 서버가
쥐고 있으므로 **API 요청·응답 필드는 바뀌지 않는다** (`CONTRACT.md` 무변경).

### 목적이 바꾸는 것과 바꾸지 않는 것 ⚠️

| | 목적의 영향 |
| --- | --- |
| `verdict`·`evidence`에서 **먼저 말할 지표** | ○ |
| `prescription`의 **축** | ○ |
| `next_goal_text`의 **강조점** | ○ |
| `recovery_note`의 방향 | ○ |
| **`next_target_min/max` 수치** | **✕** |
| 판정·downshift·업시프트 규칙 | **✕** |

**수치는 목적으로 바꾸지 않는다.** 다음 목표는 §2 업시프트 조건과 §8 하향 결과에서만
나온다. 목적까지 수치에 개입하면 같은 러닝이 목적에 따라 다른 목표를 내놓게 되고,
그 순간 "이 사람이 실제로 유지한 리듬"이라는 근거가 사라진다.

### 목적별 축

| `running_purpose` | 먼저 보는 지표 | `prescription` 축 | `next_goal_text` 강조 |
| --- | --- | --- | --- |
| `COMPLETE` | `completed`, Rhythm Score | 초반 과속 억제 | **끊지 않고 완주** |
| `HABIT` | 러닝 간격, 주간 횟수 | **다음 러닝 시점** | 이번 주 `n`회 중 `m`회 |
| `WEIGHT` | `active_duration_sec` | **지속 시간 늘리기** (강도 아님) | 총 러닝 시간 |
| `FITNESS` | Late Drop Rate | 후반 유지력 | 지속 시간 + 안정 구간 |
| `PERFORMANCE` | Rhythm Score, 평균 페이스 | 리듬 일관성 | 안정 구간 비율 |

- `HABIT`은 유일하게 **러닝 밖(다음 러닝 시점)** 을 처방한다. 한 번의 러닝을 잘하는 것보다
  다시 나가는 것이 목적이기 때문이다. 주간 횟수는 `users.weekly_goal_count`를 쓴다.
- `WEIGHT`에 강도를 올리라고 하지 않는다. 오래 편하게 달리는 것이 목적이라
  초반 과속의 대가가 가장 큰 조합이다(§2에서 목표를 낮게 잡는 것과 같은 이유).

### 금지

- **체중·칼로리·감량 수치를 생성하지 않는다.** `WEIGHT`여도 마찬가지다. 신체 정보는
  선택 입력이고 러닝 데이터로 체중 변화를 추정할 근거가 없다. 의료·영양 조언도 금지(§`CONTRACT.md`).
- 목적을 이유로 **더 빨리·더 자주 달리라고 재촉하지 않는다.** 페르소나의 Pain Point가
  *"목표를 달성하지 못하면 러닝 자체를 실패로 느낀다"*이다(§9).
- 목적이 없거나(`null`) 온보딩을 건너뛴 사용자는 `COMPLETE`로 간주한다(§2 기본값).
- 리커버리로 끝난 러닝은 목적과 무관하게 **§9의 원칙이 우선한다** — 실패로 표현하지 않는다.

### 폴백에도 같은 규칙을 적용한다

LLM이 죽어도 목적별 분기는 유지된다. 룰베이스는 위 표의 **`prescription` 축 한 문장**만
목적별로 바꾸고, 나머지는 기존 폴백 문구를 그대로 쓴다. 가설(`hypothesis`)은 폴백에서
생성하지 않는다는 규칙이 여기서도 유효하다(`CONTRACT.md`).
