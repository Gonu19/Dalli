...
def test_structured_llm_success_uses_safe_summary_and_no_retries() -> None:
    ...
    assert fake.kwargs["instructions"] == LLM_REPORT_INSTRUCTIONS_V2_2
    assert "문장은 모두 자연스러운 한국어" in LLM_REPORT_INSTRUCTIONS_V2_2
    assert "evidence는 핵심 관찰 수치 1~3개" in LLM_REPORT_INSTRUCTIONS_V2_2
    assert "prescription은 다음 러닝에서 할 행동 한 가지만" in LLM_REPORT_INSTRUCTIONS_V2_2
    assert "required_limitation 값을 그대로 복사" in LLM_REPORT_INSTRUCTIONS_V2_2
    assert "event_flags는 서버가 안전하게 요약한 사건 플래그" in LLM_REPORT_INSTRUCTIONS_V2_2
    assert "허용 형식만 사용하세요" in LLM_REPORT_INSTRUCTIONS_V2_2
